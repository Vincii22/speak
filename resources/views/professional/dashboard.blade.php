    ...
    <div>
        <!-- Dashboard content goes here -->
         @section('professional_content')
            <div class="flex flex-wrap gap-8 justify-around px-20 pb-10 pt-5">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let bookedDates = []; // Store the booked dates for the logged-in professional
    ...
    // Fetch reserved dates for the logged-in professional
    function fetchReservedDates() {
        fetch('{{ route('schedule.reservedDatesForDashboard') }}', {
            method: 'POST',
            headers: {
        ...
        .then(reservedDates => {
            bookedDates = reservedDates; // Update booked dates with the response
            renderCalendar(); // Re-render the calendar with the updated dates
        })
        .catch(error => {
            console.error('Error fetching reserved dates for dashboard:', error);
        });
    }
    ...
    // Initial fetch of reserved dates for the logged-in professional
    fetchReservedDates();
